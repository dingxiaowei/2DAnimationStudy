...
namespace FSM
{
    ...
    public abstract class AbstractState : IState
    {
        ...
        /// </summary>
        public IState PeekChildren()
        {
            if (activeChildren.Count > 0)
            {
                return activeChildren.Peek();
            }
            return null;
        }
        ...
        public void ChangeState(string stateName)
        {
            ...
            if (activeChildren.Count > 0)
            {
                if (activeChildren.Peek() == newState)
                {
                   //won't change to the same state
                    return;
                }
            ...
            BDebug.Log("change to new state!" + stateName);
#endif
...
#if DEBUG_STATE_MACHINE
            BDebug.Log("enter to child state!" + stateName);
#endif
            activeChildren.Push(newState);
            newState.Enter();
        }
        ...
        /// <summary>
        /// Get a child state by state name
        /// </summary>
        public IState GetChildState(string stateName)
        {
            IState newState;
            if (!children.TryGetValue(stateName, out newState))
            {
