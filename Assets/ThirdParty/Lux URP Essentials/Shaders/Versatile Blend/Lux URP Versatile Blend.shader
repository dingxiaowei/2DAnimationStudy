...
// Transparent shader: So we have to use the original shadow cascades and not the screen space shadow texture
...
//                  half cascadeIndex = ComputeCascadeIndex(output.positionWS);
//                  output.shadowCoord = mul(_MainLightWorldToShadow[cascadeIndex], float4(output.positionWS, 1.0));
                #endif
        ...
        //  Fragment shader and functions
            ...
            //  We have to calculate the shadowCoord per pixel :(
                //#if defined(_MAIN_LIGHT_SHADOWS) && !defined(_RECEIVE_SHADOWS_OFF)
